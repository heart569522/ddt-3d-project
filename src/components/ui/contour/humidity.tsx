...
import { IHumidityContour, ITemperatureContour } from "@/types/model";
...
interface Props {
  data: IHumidityContour;
}

export default function Humidity({ data }: Props) {
  const { humidityData, maxHumidity, minHumidity, contourInterval } = data;
  ...
  const dataContour: Partial<Plotly.PlotData>[] = [
    {
      ...
      z: humidityData,
      type: "contour",
      colorscale: configs.colorScale.humidity,
      zmin: Math.min(...humidityData),
      zmax: Math.max(...humidityData),
      ncontours: 50,
      opacity: 0.7,
      // showscale: true,
    },
      ...
      >
        <Card className="transition bg-secondary/90 cursor-default">
          <CardHeader className="pb-4">
            ...
            <CardTitle className="text-2xl sm:text-3xl md:text-4xl">
              {maxHumidity?.toFixed(configs.numberOfDecimal)}&nbsp;%
            </CardTitle>
          </CardHeader>
        ...
        <Card className="transition bg-secondary/90 cursor-default">
          <CardHeader className="pb-4">
            ...
            <CardTitle className="text-2xl sm:text-3xl md:text-4xl">
              {minHumidity?.toFixed(configs.numberOfDecimal)}&nbsp;%
            </CardTitle>
          </CardHeader>
          <CardFooter></CardFooter>
        </Card>
